...
@stop
@section('content')
<form action="{{route('standkomp.update', $standarkompetensi)}}" method="post">
    @method('PUT')
  @csrf
  <div class="row">
    <div class="col-12">
...
@error('standarkompetensi') is-invalid @enderror" id="standarkompetensi" placeholder="Standar Kompetensi"
              name="standarkompetensi" value="{{$standarkompetensi->standarkompetensi ?? old('standarkompetensi')}}">
            @error('standarkompetensi') <span class="textdanger">{{$message}}</span> @enderror
          </div>
          <div class="form-group">
            <div class="input-group mb-3">
              <input type="hidden" name="kdbidstudi" id="kdbidstudi" value="{{
               $standarkompetensi-> fbidstudi-> id ?? old('kdbidstudi')}}">
              <input type="text" class="form-control
@error('bidangstudi') is-invalid @enderror" placeholder="Bidang Studi" id="bidangstudi" name="bidangstudi"
                value="{{ $standarkompetensi->fbidstudi->bidangstudi ?? old('bidangstudi')}}" arialabel="Bidang Studi" aria-describedby="cari" readonly>
